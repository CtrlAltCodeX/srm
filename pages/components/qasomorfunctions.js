...
function myFunction() {
	...
	var focusSet = false;
	if (!$('#riskcontext').val()) {
		if ($("#riskcontext").parent().next(".validation").length == 0) // only add if not added
		{
			$("#riskcontext").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter riskcontext</div>");
		}
	...
	if (!$('#riskconsequence').val()) {
		if ($("#riskconsequence").parent().next(".validation").length == 0) // only add if not added
		{
			$("#riskconsequence").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter riskconsequence</div>");
		}
	...
	if (!$('#rhood').val()) {
		if ($("#rhood").parent().next(".validation").length == 0) // only add if not added
		{
			$("#rhood").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter risk Likelihood</div>");
		}
	...
	if (!$('#recommendeda').val()) {
		if ($("#recommendeda").parent().next(".validation").length == 0) // only add if not added
		{
			$("#recommendeda").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter recommended action</div>");
		}
	...
	if (!$('#ohsref').val()) {
		if ($("#ohsref").parent().next(".validation").length == 0) // only add if not added
		{
			$("#ohsref").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter some reference</div>");
		}
	...
	if (!$('#hitype').val()) {
		if ($("#hitype").parent().next(".validation").length == 0) // only add if not added
		{
			$("#hitype").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please select related to</div>");
		}
	...
	if (!$('#pct').val()) {
		if ($("#pct").parent().next(".validation").length == 0) // only add if not added
		{
			$("#pct").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please select potential cause type</div>");
		}
	...
	if (!$('#pcd').val()) {
		if ($("#pcd").parent().next(".validation").length == 0) // only add if not added
		{
			$("#pcd").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter some potential cause description</div>");
		}
	...
	if (!$('#ect').val()) {
		if ($("#ect").parent().next(".validation").length == 0) // only add if not added
		{
			$("#ect").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please select effect type</div>");
		}
	...
	if (!$('#ecd').val()) {
		if ($("#ecd").parent().next(".validation").length == 0) // only add if not added
		{
			$("#ecd").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please select effect type</div>");
		}
	...
	if (!$('#hazorinc').val()) {
		if ($("#hazorinc").parent().next(".validation").length == 0) // only add if not added
		{
			$("#hazorinc").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please select if either hazard or incident</div>");
		}
	...
	if (!$('#certificatehazard').val()) {
		if ($("#certificatehazard").parent().next(".validation").length == 0) // only add if not added
		{
			$("#certificatehazard").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please select hazard certificate</div>");
		}
	...
	var dataString = { riskcontext: riskcontext, riskowner: riskowner, id: id, riskconsequence: riskconsequence, rhood: rhood, rl: rl, rew: rew, recommendeda: recommendeda, ohsref: ohsref, qaso: qaso, hitype: hitype, potcausetype: potcausetype, potcaused: potcaused, effecttype: effecttype, effectd: effectd, hazorinc: hazorinc, certificatehazard: certificatehazard };
	if (!(riskcontext == '') && !(riskconsequence == '') && !(rhood == '') && !(recommendeda == '') && !(ohsref == '') && !(hitype == '') && !(potcausetype == '') && !(potcaused == '') && !(effecttype == '') && !(effectd == '') && !(hazorinc == '') && !(certificatehazard == '')) {
...
function approveDisapprove() /* DONE */ {
	...
	if (!(updatecontent == '') && !(updateuser == '')) {
		setTimeout(function () { $('#UPDATEAUDIT').modal('hide') }, 3)
		// $('#submitting').show();
		...
		// }
		// else {
		var dataStringro = { id: IDAUDIT, ro: '', updatecontent: '', audititemno: '', updateuser: '', status: status, FileName: "", extension: "" };
...
function update() {
	...
	if (radio === "1") {
		...
		if (!$('#resriskconsequenceq').val()) {
			if ($("#resriskconsequenceq").parent().next(".validation").length == 0) // only add if not added
			{
				$("#resriskconsequenceq").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter Residual Risk Consequence</div>");
			}
		...
		if (!$('#resrhoodq').val()) {
			if ($("#resrhoodq").parent().next(".validation").length == 0) // only add if not added
			{
				$("#resrhoodq").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter Residual Risk Likelihood</div>");
			}
		...
		if (!$('#resrlq').val()) {
			if ($("#resrlq").parent().next(".validation").length == 0) // only add if not added
			{
				$("#resrlq").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter Residual Risk Level</div>");
			}
		...
		if (!$('#updatecontent').val()) {
			...
			{
				$("#updatecontent").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter some final comments</div>");
			}
		...
		if (!$('#updateuser').val()) {
			...
			{
				$("#updateuser").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter your name or title</div>");
			}
		...
		if (!$('#resrating').val()) {
			if ($("#resrating").parent().next(".validation").length == 0) // only add if not added
			{
				$("#resrating").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter Residual Risk Rating</div>");
			}
		...
		if (!(updatecontent == '') && !(resrating == '') && !(updateuser == '') && !(resriskconsequenceq == '') && !(resrhoodq == '') && !(resrlq == '')) {

			setTimeout(function () { $('#myMODALTHREE').modal('hide') }, 3)
			$('#submitting').show();

			var status = "CLOSED";
			var dataStringro = { id: id, updatecontent: updatecontent, updateuser: updateuser, status: status, resriskconsequenceq: resriskconsequenceq, resrhoodq: resrhoodq, resrlq: resrlq, resrating: resrating };
			$.ajax({
				type: "POST",
		...
		if (!$('#updatecontent').val()) {
			if ($("#updatecontent").parent().next(".validation").length == 0) // only add if not added
			{
				$("#updatecontent").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter some comments</div>");
			}
		...
		if (!(updatecontent == '') && !(updateuser == '')) {
			...
			var status = "OPEN";
			var dataStringro = { id: id, updatecontent: updatecontent, updateuser: updateuser, status: status, targetdate: targetdate };
			$.ajax({
				type: "POST",
...
function updatemor() {
	...
	setTimeout(function () { $('#MORMODALQASO').modal('hide') }, 3)
	$('#submitting').show();
...
function updateaudit() {
	...
	setTimeout(function () { $('#QASOAUDITMODAL').modal('hide') }, 3)
	$('#submitting').show();
...
function updateauditall() {
	...
	if (files != null) {
		...
		var dataStringro = { id: IDCOMP, updatecontent: updatecontent, updateuser: updateuser, status: status, FileName: files, FileSize: "NOT NEEDED", FileType: "NOT NEEDED", extension: files };
	}
	else {

		var dataStringro = { id: IDCOMP, updatecontent: updatecontent, updateuser: updateuser, status: status, FileName: "", FileSize: "", FileType: "", extension: "" };
...
function qasotransfer() {
	setTimeout(function () { $('#exampleModalTransfer').modal('hide') }, 3)
	$('#submitting').show();
	...
	var dataStringro = { id: IDT, qasoransfer: qasoransfer, reasonqaso: reasonqaso, pqaso: qaso };
...
function updateaudit() /* DONE */ {
	...
	if (!$('#audititemno').val()) {
		if ($("#audititemno").parent().next(".validation").length == 0) // only add if not added
		{
			$("#audititemno").parent().after("<div class='validation' style='color:red;margin-bottom: 20px;'>Please enter update contents</div>");
		}
